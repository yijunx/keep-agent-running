...
class LLMConfig(BaseModel):
    ...
    base_url: str = Field(
        ..., description="Base URL of the vLLM server (e.g., http://10.4.33.17:80/v1)"
    )
    api_key: str = Field(
        default="dummy-key", description="API key (not used for most vLLM deployments)"
    )
    model_name: str = Field(
        ...,
        description="Name of the model to use (e.g., inarikami/DeepSeek-R1-Distill-Qwen-32B-AWQ)",
    )
...
class PydanticConverter:
    ...
    def convert_into_pydantic_model(self, output: str, pydantic_model: Type[BaseModel]) -> BaseModel:
...
4. Ensure all required fields are present"""
    ...
    def convert_into_pydantic_model_list(self, output: str, pydantic_model: Type[BaseModel]) -> List[BaseModel]:
        """
...
if __name__ == "__main__":
    ...
    person = pydantic_converter.convert_into_pydantic_model(text, Person)
    ...
    """
    people = pydantic_converter.convert_into_pydantic_model_list(text_multiple, Person)
